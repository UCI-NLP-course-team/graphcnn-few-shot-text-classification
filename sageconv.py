...
class SAGEConv(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        if self._aggre_type == 'mean':
            graph.ndata['h'] = feat
            graph.update_all(fn.u_mul_e('h', 'weight', 'm'), fn.mean('m', 'neigh'))
            ...
            graph.update_all(fn.u_mul_e('h', 'weight', 'm'), fn.max('m', 'neigh'))
            h_neigh = graph.ndata['neigh']
        elif self._aggre_type == 'lstm':
            graph.ndata['h'] = feat
            graph.update_all(fn.u_mul_e('h', 'weight', 'm'), self._lstm_reducer)
            h_neigh = graph.ndata['neigh']
        else:
